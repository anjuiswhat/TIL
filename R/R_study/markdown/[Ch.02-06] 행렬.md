...
- byrow 옵션을 생략할 경우, 기본값 FALSE로 자동 적용되어 행렬 값이 좌측 열부터 채워진다. (열 우선column major).
- 행렬 값을 위쪽 행부터 채우고 싶다면(행 우선row major) byrow=TRUE를 지정한다.
...
- 이때 ridx나 cidx에 벡터를 사용해 여러 값을 지정할 수 있다.
- ridx나 cidx 중 하나를 생략하면 전체 행 또는 열을 의미함
...
### solve(  ) : a %*% x = b 일 때, x를 계산

- 반환 값은 a %*% x = b에서 x다.
- b를 지정하지 않으면 a의 역행렬을 구하게 된다.

다음은 행렬 x의 역행렬을 구한 뒤 x와 곱해 그 결과가 단위행렬Identity Matrix이 되는지 확인하는 예다.
